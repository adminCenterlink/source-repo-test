import type { ModuleEntry, Registry } from "./registry.js";
...
export function resolveDependencies(
  ...
  function visit(name: string, stack: string[]): void {
    ...
    if (!mod) {
      throw new Error(
        `'${stack[stack.length - 1] ?? "(root)"}'의 의존 모듈 '${name}'을 registry에서 찾을 수 없습니다.`,
      );
    }
    visiting.add(name);
...
export function mergeNpmDeps(modules: ModuleEntry[]): Record<string, string> {
