...
import { readLock, type InstalledModule } from "../lock.js";
...
interface FileDiff {
  ...
  /** lock에 기록된 hash */
  lockHash: string;
  /** 현재 타겟 파일 hash (없으면 null) */
  targetHash: string | null;
  /** 현재 source 파일 hash (없으면 null) */
 ...
 */
export async function runDiff(moduleName: string | undefined, options: DiffOptions): Promise<void> {
  ...
  const modulesToCheck: InstalledModule[] = [];
  if (moduleName) {
    const found = Object.values(lock.modules).find(
      (m) => m.name === moduleName || m.asPath === moduleName,
    );
    if (!found) {
  ...
  console.log(chalk.cyan.bold("\n=== 변경 요약 ==="));
  console.log(
    `  변경된 파일 합계: ${totalChanged}개 (개발팀 수정 ${totalLocal}, source 갱신 ${totalUpstream}, 양쪽 ${totalBoth}, 삭제 ${totalMissing})`,
  );
  if (totalChanged === 0) {
