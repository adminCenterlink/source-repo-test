...
import { getModulesByCategory, loadRegistry, type ModuleEntry } from "../registry.js";
import { resolveDependencies } from "../resolver.js";
import { readLock, writeLock, type InstalledModule, type LockFile } from "../lock.js";
 ...
 */
export async function runAdd(moduleName: string | undefined, options: AddOptions): Promise<void> {
  ...
  if (options.category) {
    ...
    console.log(
      chalk.cyan(`→ '${options.category}' 카테고리: ${rootModules.length}개 모듈 추가`),
    );
  } else if (moduleName) {
  ...
  const allResolved = resolveDependencies(
    registry,
    rootModules.map((m) => m.name),
  );
  resolveSpinner.succeed(`총 ${allResolved.length}개 모듈 (의존 포함)`);

  // 기존 lock 로드
  const lock: LockFile =
    (await readLock(targetRoot)) ?? {
  ...
  for (const mod of allResolved) {
    // --as 옵션은 root 모듈에만 적용
    const isRoot =
      rootModules.length === 1 && mod.name === rootModules[0].name && Boolean(options.as);
