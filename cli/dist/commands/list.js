...
function printGroupedByCategory(modules, registry) {
    ...
    for (const [cat, mods] of [...byCategory.entries()].sort(([a], [b]) => a.localeCompare(b))) {
        ...
        for (const m of mods.sort((a, b) => a.name.localeCompare(b.name))) {
            ...
            console.log(`  ${chalk.white(m.name)}  ${lvlColor(`[${m.customizationLevel}]`)}  ${chalk.gray(m.description)}`);
        }
    }
}
function printModuleLine(m) {
    const lvlColor = customizationColor(m.customizationLevel);
    const tags = m.tags.length > 0 ? chalk.gray(`#${m.tags.join(" #")}`) : "";
    console.log(`  ${chalk.white(m.name)}  ${lvlColor(`[${m.customizationLevel}]`)}  ${chalk.gray(m.description)}  ${tags}`);
}
...
function customizationColor(level) {
    switch (level) {
        case "low":
            return chalk.green;
        case "medium":
