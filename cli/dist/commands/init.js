...
export async function runInit(options) {
    ...
    const targetDir = path.resolve(options.targetDir ?? projectMap.options?.targetDir ?? `./${projectMap.projectName}`);
    ...
    // 8) npm install
    const skipInstall = options.skipInstall ?? projectMap.options?.skipNpmInstall ?? false;
    if (skipInstall) {
...
async function readProjectMap(filePath) {
    const abs = path.resolve(filePath);
    const content = await fs.readFile(abs, "utf-8");
    return JSON.parse(content);
}
async function runInteractiveInit(registry) {
    ...
    for (const [categoryKey, category] of Object.entries(registry.categories)) {
        const modulesInCategory = Object.values(registry.modules).filter((m) => m.category === categoryKey);
...
async function copyTemplates(sourceRoot, targetDir, projectName) {
...
async function copyDirRecursive(src, dst, projectName) {
...
async function mergePackageJson(targetDir, registry, resolved, projectName) {
...
async function generateRoutes(targetDir, registry, pages) {
...
async function generateHomePage(targetDir, pages) {
...
async function copyDepCruiserConfig(sourceRoot, targetDir) {
    const candidates = [
    ...
    return new Promise((resolve, reject) => {
        ...
        proc.on("exit", (code) => {
            if (code === 0)
                ...
                reject(new Error(`${cmd} ${args.join(" ")} 실패 (exit ${code})`));
        });
    });
}
